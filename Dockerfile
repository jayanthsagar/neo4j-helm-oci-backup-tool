...
RUN echo "deb [signed-by=/usr/share/keyrings/neo4j.gpg] https://debian.neo4j.com stable 4.4" | tee -a /etc/apt/sources.list.d/neo4j.list
...
RUN apt-get update && apt-get install -y neo4j-enterprise=1:4.4.10 unzip less && rm -rf /var/lib/apt/lists/*
